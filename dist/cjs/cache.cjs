...
const weakCache = () => {
	...
	const registry = new FinalizationRegistry((heldValue) => {
		...
		if (ref?.deref && !ref.deref()) {
			...
			if (process.env.NODE_ENV !== "production") console.debug(`Cleared cache for key: ${heldValue}`);
		}
	});
	return {
		...
		set: (key, value, strong) => {
			if (strong) cacheMap.set(key, value);
			else {
...
const createCache = () => typeof WeakRef === "function" ? weakCache() : /* @__PURE__ */ new Map();
//#endregion
