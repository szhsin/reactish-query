...
const createQueryClient = ({ middleware } = {}) => {
	const state = (0, reactish_state.stateBuilder)(middleware);
	const cache = require_cache.createCache();
	const getCacheEntry = (queryMeta) => cache.get(require_queryCacheUtils.getStrCacheKey(queryMeta));
	...
	const resolveCacheEntry = (queryMeta, queryFn, shouldPersist, strQueryKey) => {
		const strCacheKey = require_queryCacheUtils.getStrCacheKey(queryMeta, strQueryKey);
	...
	return {
		...
		*/
		fetch: ({ queryFn, staleTime, ...queryMeta }) => {
			...
			if (require_queryCacheUtils.isDataFresh(cacheEntry, staleTime)) return { data: cacheEntry[0].d.get() };
		...
		*/
		invalidate: (queryMeta) => {
			const cacheEntry = getCacheEntry(queryMeta);
			if (cacheEntry) return require_queryCacheUtils.fetchCacheEntry(queryMeta, cacheEntry);
		}
	};
};
